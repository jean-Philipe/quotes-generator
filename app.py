...
def build_drawtext_filters(lines, font_path, author, author_font_path, font_size=38, spacing=10):
    ...
    if author:
        ...
        author_drawtext = (
            ...
            f"text='{author_line}':"
            f"fontcolor=white:fontsize={font_size - 10}:"
...
def generate_video():
    ...
    file_url = get_direct_download_url(request.form.get('fileUrl'))
    audio_url = get_direct_download_url(request.form.get('audioUrl'))

    if not quote or not file_url or not audio_url:
        return {"error": "Missing 'quote', 'fileUrl' or 'audioUrl'"}, 400

    image_path = f"/tmp/{uuid4()}.png"
    audio_path = f"/tmp/{uuid4()}.mp3"
    ...
    try:
        ...
        return {"error": f"Failed to download files: {str(e)}"}, 500

    drawtext_filter = build_drawtext_filters(
        wrap_text_lines(quote, max_chars_per_line=25),
...
def generate_video_from_clip():
    ...
    drawtext_filter = build_drawtext_filters(
        wrap_text_lines(quote, max_chars_per_line=30),
        "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",
        author,
        "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
    )
    ...
    ffmpeg_cmd = [
        'ffmpeg', '-stream_loop', '-1', '-i', input_video_path, '-i', audio_path,
        '-vf', f"scale=720:1280,{drawtext_filter}",
        '-c:v', 'libx264', '-pix_fmt', 'yuv420p',
        '-c:a', 'aac', '-shortest', '-y', output_video_path
    ]
